...
def botResponse(user_input,is_startup=True):
    ...
    if is_startup:
        response = "Hi, I'm happy to have you here \nI have a lot to explore about Email Templates"
        is_startup = False
        return response
...
def get_text():
    input_text = st.text_input("Email Keyword Search: ","type here")
    df_input = pd.DataFrame([input_text],columns=['questions'])
    return df_input 



st.sidebar.title("Email Templates")
st.title("""
Email Templates  
Email Tempaltes is an NLP conversational template trainned on Email corpus using  CNN achitecture
""")
...
st.text_area("Email Template:", value=response, height=200, max_chars=None, key=None)
